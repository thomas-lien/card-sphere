...
if (-not (Test-Path ".env.local")) {
    if (Test-Path ".env.example") {
        ...
        Write-Host "⚠️  Don't forget to add your WalletConnect Project ID to .env.local" -ForegroundColor Yellow
    } else {
        Write-Host "⚠️  Warning: .env.example not found. Please create .env.local manually" -ForegroundColor Red
    }
}

Write-Host "`n✅ Setup complete!" -ForegroundColor Green
Write-Host "📝 Next steps:" -ForegroundColor Cyan
Write-Host "1. Make sure your .env.local has the correct WalletConnect Project ID"
